...
fn get_stable_version(name: &str, env: &str) {
    ...
    let output = match output {
        ...
        Err(e) => {
            eprintln!(
                "cargo:warning=Failed to execute git describe for {}: {}. Setting empty version.",
                name, e
            );
    ...
    if output.status.success() {
        ...
        if let Some(version) = version_tag.strip_prefix(&prefix) {
            ...
            eprintln!(
                "cargo:warning=Found tag '{}' for crate '{}' but it did not have the expected prefix '{}'. Setting empty version.",
                version_tag, name, prefix
            );
            println!("cargo:rustc-env={}=", env);
        }
    } else {
        ...
        if stderr.contains("No names found") {
            ...
            println!(
                "cargo:warning=No stable tag found for crate '{}'. Defaulting to empty version. Use --dev for create command.",
                name
            );
        } else {
            eprintln!(
                "cargo:warning=git describe for {} failed: {}. Setting empty version.",
                name, stderr
            );
        }
        println!("cargo:rustc-env={}=", env);
