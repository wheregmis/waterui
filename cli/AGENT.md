...
- Maintain **fast fail** semantics: if the Rust cdylib or FFI exports are missing required symbols (e.g., hot reload hooks), let the build/runtime crash with a clear error rather than adding “best-effort” fallbacks. Mismatched binaries must be rebuilt instead of silently skipping functionality.
