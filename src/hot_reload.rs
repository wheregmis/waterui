...
fn create_library(data: &[u8]) -> PathBuf {
    ...
    if !hot_reload_dir.exists() {
        std::fs::create_dir_all(&hot_reload_dir).expect("Failed to create hot_reload directory");
    }
    ...
    N: for<'writer> FormatFields<'writer> + 'static,
{
    fn format_event(
        ...
        if is_panic_event(event.metadata()) {
            ...
            // Default formatting for non-panic events
    ...
    S: tracing::Subscriber + for<'span> LookupSpan<'span>,
{
    fn on_event(&self, event: &tracing::Event<'_>, _ctx: Context<'_, S>) {
        ...
        // Format the event message directly
...
fn log_panic_via_tracing(info: &PanicHookInfo) {
...
impl ClientEvent {
    fn from_panic(info: &PanicHookInfo) -> Self {
...
impl From<&PanicHookInfo<'_>> for PanicReport {
...
fn endpoint_from_env() -> Option<HotReloadEndpoint> {
    let port = std::env::var("WATERUI_HOT_RELOAD_PORT")
        ...
        .parse::<u16>()
        .ok()?;
    let host = std::env::var("WATERUI_HOT_RELOAD_HOST").unwrap_or_else(|_| "127.0.0.1".to_string());
