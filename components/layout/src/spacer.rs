...
use crate::{Layout, ProposalSize, Size, Rect, ChildMetadata};

/// A flexible space component that expands to fill available space.
/// 
/// Spacers are commonly used in layouts to push other components apart
/// or to create flexible spacing that adapts to the container size.
pub struct Spacer {
    min_length: f64,
}
...
impl Layout for SpacerLayout {
    ...
    fn size(&mut self, parent: ProposalSize, _children: &[ChildMetadata]) -> Size {
        ...
        let height = parent.height.unwrap_or(self.min_length).max(self.min_length);
        
        Size::new(width, height)
    }

    fn place(&mut self, _bound: Rect, _proposal: ProposalSize, _children: &[ChildMetadata]) -> Vec<Rect> {
...
pub fn spacer_min(min_length: f64) -> Spacer {
